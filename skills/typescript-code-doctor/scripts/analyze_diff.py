...
detectors on each changed TypeScript file and keeps only findings that land on
lines the diff touched.

Whole-tree detectors (import cycles, dependency hygiene, dead code, duplication,
over-engineering, untested modules, tsconfig) need the full project to be
correct, so they are deliberately NOT part of the diff lens — run them with
analyze_all.py against the whole repo.
...
def _expand_to_definitions(filepath, lines):
    """Changed lines plus the header line of every declaration whose body
    intersects them.
    ...
    try:
        ...
        tsfile = parse_file(Path(filepath))
    except (OSError, ValueError, ImportError):
        return accepted
    except TsSyntaxError:
        return accepted
    spans = [(f.line, tsfile.line_of(f.body_close) if f.has_body else f.line)
             for f in tsfile.functions]
    spans += [(k.line, tsfile.line_of(k.body_close)) for k in tsfile.classes]
    spans += [(t.line, tsfile.line_of(t.end)) for t in tsfile.types]
...
def collect(base, all_lines, scripts=None):
    ...
    for filepath, lines in files.items():
        ...
        # declarations without being accepted themselves.
        seeds = set()
