...
|---|---|---|
| `01-summary.html` | Summary | Behavioral summary from the diff itself, opening with who made the change (`authors` from the analyze_diff summary); claimed-vs-actual; what didn't change |
...
python $SKILL/scripts/assemble.py --tabs-dir $TABS \
  ...
  --meta "by <authors, most commits first> · base <base-ref> @ <sha> → <head> · +<adds>/−<dels> · generated <date>" \
  --footer "Generated by pr-visualization. Automated tabs: Footprint, Contracts &amp; Tests (incl. prompt/model contracts), Blast Radius (heuristic, name-based; incl. loaders of changed resources). Judgment tabs authored from diff reading — verify citations before acting."
```
