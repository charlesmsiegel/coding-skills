"""The record contract in rust-code-doctor's common.py.
...
def test_finding_requires_a_suggestion(common):
    with pytest.raises(common.SchemaError, match="suggestion"):
        common.Finding(file="a.rs", line=1, smell_type="x", description="d", suggestion="")


def test_finding_may_not_carry_benign_explanations(common):
    with pytest.raises(common.SchemaError, match="also_caused_by"):
        common.Finding(file="a.rs", line=1, smell_type="x", description="d",
...
def test_finding_is_frozen_and_collections_are_tuples(common):
    candidate = common.Finding(file="a.rs", line=1, smell_type="x", description="d",
...
def test_finding_survives_json_round_trip(common):
    original = common.Finding(file="a.rs", line=2, smell_type="y", description="d",
                              kind="candidate", also_caused_by=["it is an entry point"],
                              related_lines=[3, 4])
...
def test_reporter_add_makes_a_finding_with_the_snippet(common):
    reporter = common.Reporter(_File(Path("a.rs")), set())
...
def test_reporter_candidate_honours_ignore(common):
    reporter = common.Reporter(_File(Path("a.rs")), {"lead"})
...
def test_sort_puts_findings_before_candidates(common):
    low_finding = common.Finding(file="a.rs", line=9, smell_type="x", description="d",
                                 suggestion="fix", severity="low")
    high_candidate = common.Finding(file="a.rs", line=1, smell_type="y", description="d",
...
def test_text_report_separates_candidates(common, capsys):
    finding = common.Finding(file="a.rs", line=1, smell_type="x", description="a defect",
                             suggestion="fix it")
    candidate = common.Finding(file="a.rs", line=2, smell_type="y", description="a lead",
